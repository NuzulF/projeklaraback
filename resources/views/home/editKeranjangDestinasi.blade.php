...
<body>
    ...
    <section id="pesanTiket" class="blog pesanTiket">
        <form id="pesan" action="{{ url('keranjang/edit/'.$keranjang->id.'/destinasi') }}"
            method="POST">
            @csrf
            <input type="hidden" name="destinasi" value="{{ $destinasi->id }}">
            <div class="row w-100 m-auto justify-content-around">
                <div class="col-12 col-md-7 container p-0 m-2" data-aos="fade-up">
                    <div class="inputTiket" style="width:auto;">
                        <h4>Edit Tiket Untuk {{ $destinasi->nama_destinasi }}</h4>
                        <div class="row g-3 m-0">
                            <div class="col-12">
                                <label class="form-label">Tiket masuk : @currency($destinasi->htm_destinasi)</label>
                            </div>
                            <div class="col-12">
                                <label for="inputDestinasi" class="form-label">Pilih paket wahana</label>
                                <div class="form-check">
                                    @if($keranjang->paketWahana()->wherePivot('index',1)->count() == 0)
                                        <input class="form-check-input w-auto" type="checkbox" value="0" name="wahana[]" id="defaultCheck0" checked>
                                    @else
                                        <input class="form-check-input w-auto" type="checkbox" value="0" name="wahana[]" id="defaultCheck0">
                                    @endif
                                ...
                                <div class="form-check">
                                        ...
                                        @foreach($keranjang->paketWahana as $kpak)
                                            @if ($kpak->pivot->index == 1 && $pak->id == $kpak->id)
                                                <input class="form-check-input w-auto {{ $pak->id }} == {{ $kpak->id }}" type="checkbox" value="{{ $pak->id }}" name="wahana[]" id="defaultCheck{{ $pak->id }}" checked>
                                                @php
                                                    $selected = true;
                                                @endphp
                                            @endif
                                        @endforeach
                                        @if(!$selected)
                                            <input class="form-check-input w-auto" type="checkbox" value="{{ $pak->id }}" name="wahana[]" id="defaultCheck{{ $pak->id }}">
                                        @endif
                                        <label class="form-check-label" for="defaultCheck{{ $pak->id }}">
