...
<body>
    ...
    <section id="pesanTiket" class="blog pesanTiket">
        <form id="pesan" action="{{ url('keranjang/edit/' . $keranjang->id.'/paket') }}"
            method="POST">
            @csrf
            ...
            <div class="row w-100 m-auto justify-content-around">
                <div class="col-12 col-md-7 container p-0 m-2" data-aos="fade-up">
                    <div class="inputTiket" style="width:auto;">
                        <h4>Edit Tiket Untuk {{ $paket->nama_paket }}</h4>

                        <div class="col-12">
                            <label class="form-label">Harga total tiket masuk: @currency($paket->harga_paket)</label>
                        </div>

                        @foreach($paket->destinasi as $destinasi)
                            <div class="row g-3 m-0">
                                <div class="col-12">
                                    <label for="inputDestinasi" class="form-label">Pilih paket wahana {{ $destinasi->nama_destinasi }}</label>
                                    <div class="form-check">
                                        @php
                                            $null = false;
                                        @endphp
                                        @foreach($keranjang->destinasi as $destinasiKeranjang)
                                            @if ($destinasiKeranjang->pivot->index == 1 && $destinasiKeranjang->id == $destinasi->id && $destinasiKeranjang->pivot->paket_wahana_id == null)
                                                @php
                                                    $null = true;
                                                @endphp
                                                <input class="form-check-input w-auto destinasi-{{ $destinasi->id }}" type="checkbox" value="0" name="wahana[{{ $destinasi->id }}][]" id="defaultCheck{{ $destinasi->id }}-0" checked>
                                            @endif
                                        @endforeach
                                        @if(!$null)
                                            <input class="form-check-input w-auto destinasi-{{ $destinasi->id }}" type="checkbox" value="0" name="wahana[{{ $destinasi->id }}][]" id="defaultCheck{{ $destinasi->id }}-0">
                                        @endif
                                        <label class="form-check-label" for="defaultCheck1">
                                            Tidak ada
                                        </label>
                                    </div>
                                    @forelse ($destinasi->paketWahana as $pak)
                                    <div class="form-check">
                                        @php
                                            $selected = false;
                                        @endphp
                                        @foreach($keranjang->paketWahana as $kpak)
                                            @if ($kpak->pivot->index == 1 && $pak->id == $kpak->id)
                                                <input class="form-check-input w-auto destinasi-{{ $destinasi->id }}" type="checkbox" value="{{ $pak->id }}" name="wahana[{{ $destinasi->id }}][]" id="defaultCheck{{ $destinasi->id }}-{{ $pak->id }}" checked>
                                                @php
                                                    $selected = true;
                                                @endphp
                                            @endif
                                        @endforeach
                                        @if(!$selected)
                                            <input class="form-check-input w-auto destinasi-{{ $destinasi->id }}" type="checkbox" value="{{ $pak->id }}" name="wahana[{{ $destinasi->id }}][]" id="defaultCheck{{ $destinasi->id }}-{{ $pak->id }}">
                                        @endif
                                        <label class="form-check-label" for="defaultCheck2">
                                            {{ $pak->nama_paket }} (@currency($pak->harga_paket))
                                        </label>
                                        <p>
                        ...
                        <div class="row g-3 m-0">
                            @guest
                                <div class="col-12">
                                    ...
                                    <input type="text" class="form-control mb-3" id="inputNamaPemesan"
                                ...
                                <div class="col-12">
                                    ...
                                    <input type="email" class="form-control mb-3" id="inputEmailPemesan"
                                        placeholder="Masukkan Email Pemesan" value="{{ old('email_pemesan') }}" name="email_pemesan" required />
                                </div>
                                <div class="col-12">
                                    <label for="inputNomorPemesan" class="form-label">No. Telp Pemesan</label>
                                    <input type="test" class="form-control mb-3" id="inputNomorPemesan"
                                        placeholder="Masukkan Nomor Telepon Pemesan" value="{{ old('no_telp_pemesan') }}" name="no_telp_pemesan" required />
                                </div>
                            @else
                                <div class="col-12">
                                    <label for="inputNamaPemesan" class="form-label">Nama Pemesan</label>
                                ...
                                <div class="col-12">
                                    <label for="inputEmailPemesan" class="form-label">Email Pemesan</label>
                                    <input type="text" class="form-control mb-3" id="inputEmailPemesan"
                                        placeholder="{{ Auth::user()->email }}" readonly />
                                </div>
                                <div class="col-12">
                                    <label for="inputNomorPemesan" class="form-label">No. Telp Pemesan</label>
                                    <input type="test" class="form-control mb-3" id="inputNomorPemesan"
                                        placeholder="{{ Auth::user()->phone }}" readonly />
                                </div>
                            @endguest
                            <div class="col-12">
                                <label for="tanggal" class="form-label">Tanggal Pemesanan</label>
                                <input type="date" class="form-control mb-3" id="tanggal" value="{{ $keranjang->tanggal_kunjungan }}" name="tanggal" required />
                            </div>
                            <div class="col-md-12">
                                <label for="jumlah" class="form-label">Jumlah Tiket</label>
                                <input type="number" class="form-control" id="jumlah" value="{{ $keranjang->jumlah }}" name="jumlah" min="1" required />
                            </div>
                            {{-- <div class="col-md-6">
                            ...
                            <input type="submit" class="btn col-11 mr-auto ml-auto mt-5 h-25" style="background-color:#092742;color:#ffff" id="buttonLogin" value="Simpan">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        <script>
            $(document).ready(function() {
                ...
                @foreach($paket->destinasi as $destinasi)
                    $('input:checkbox.destinasi-{{ $destinasi->id }}').change(function() {
                        if ($(this).val() === "0") {
                            // Jika yang dipilih adalah checkbox dengan nilai 0, maka
                            // hilangkan centang dari checkbox lainnya
                            $('input:checkbox.destinasi-{{ $destinasi->id }}').not(this).prop('checked', false);
                        } else {
                            // Jika yang dipilih bukan checkbox dengan nilai 0, maka
                            // hilangkan centang dari checkbox dengan nilai 0
                            $('input:checkbox#defaultCheck{{ $destinasi->id }}-0').prop('checked', false);
                        }
                        if ($('input:checkbox.destinasi-{{ $destinasi->id }}:checked').length === 0) {
                            // Jika tidak ada yang terpilih, otomatis centang checkbox dengan value 0
                            $('input:checkbox#defaultCheck{{ $destinasi->id }}-0').prop('checked', true);
                        }
                    });
                @endforeach
            });
